...
export async function searchCommand(query: string, opts: SearchOptions) {
	...
	const requested = Number.parseInt(opts.limit || "10", 10);
	const safeLimit = Math.min(
		Number.isFinite(requested) && requested > 0 ? requested : 10,
		SEARCH_LIMIT_CAP,
	);
	const params = new URLSearchParams({ q: query, type, limit: String(safeLimit) });
	...
	if (type === "track" && data.tracks) {
		...
		const items = data.albums.items.map((a) => ({
			id: a.id,
			name: a.name,
			artist: (a.artists ?? []).map((ar) => ar.name ?? "").filter(Boolean).join(", "),
